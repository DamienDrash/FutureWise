<script>
  ...
  async function uploadCsv(e) {
    ...
    try {
      ...
      fd.append('file', file)
      const res = await fetch(`${API}/imports/csv`, { method: 'POST', body: fd, credentials: 'include' })
      if (!res.ok) {
...
<section class="p-6 space-y-6">
  ...
  <div class="card bg-base-100 shadow">
    <div class="card-body flex gap-4 items-end">
      ...
      <button class="btn" on:click={loadSummary}>Zusammenfassung laden</button>
    </div>
  </div>
    ...
    <div class="card-body">
      ...
      {:else}
        <div class="stats shadow">
          <div class="stat"><div class="stat-title">Tage</div><div class="stat-value">{summary?.summary?.num_days ?? 0}</div></div>
          <div class="stat"><div class="stat-title">Sessions</div><div class="stat-value">{summary?.summary?.sessions_sum ?? 0}</div></div>
          <div class="stat"><div class="stat-title">Orders</div><div class="stat-value">{summary?.summary?.orders_sum ?? 0}</div></div>
          <div class="stat"><div class="stat-title">Revenue Gross</div><div class="stat-value">{summary?.summary?.revenue_gross_sum ?? 0}</div></div>
          <div class="stat"><div class="stat-title">Revenue Net</div><div class="stat-value">{summary?.summary?.revenue_net_sum ?? 0}</div></div>
        </div>
      {/if}
    </div>
  </div>
  ...
  <div class="card bg-base-100 shadow">
    <div class="card-body">
      ...
      <input type="file" accept=".csv" class="file-input file-input-bordered" on:change={uploadCsv} disabled={uploading} />
      {#if error}
      ...
      <ul class="space-y-2">
        {#each events as ev}
          <li class="flex justify-between border-b pb-2">
            ...
            <span>{ev.inserted_count} insert · {ev.error_count} errors</span>
          </li>
        {/each}
      </ul>
    </div>
